# load packages and capture snapshot of the environment----
library(tidyverse)
...
Txi_gene <- tximport(path, 
                     type = "kallisto", 
                     tx2gene = Tx, 
                     txOut = FALSE, #How does the result change if this =FALSE vs =TRUE. True= Transcript level data.
...
Txi_gene <- tximport(path, 
                     type = "kallisto", 
                     tx2gene = Tx, 
                     txOut = FALSE, #determines whether your data represented at transcript or gene level (TRUE = Transcript level data)
                     countsFromAbundance = "lengthScaledTPM",
                     ignoreTxVersion = TRUE)
