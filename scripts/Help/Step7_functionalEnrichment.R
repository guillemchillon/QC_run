...
gost.res <- gost(rownames(myModule_up), organism = "hsapiens", correction_method = "fdr") #myModule can be used as the same way as myTopHits
...
mygostplot <- gostplot(gost.res, interactive = T, capped = T)
...
## Daniel Beitling tends to use Symbols instead of Entrez Gene ID because is what he uses normally.
...
hs_gsea %>% 
  dplyr::distinct(gs_cat, gs_subcat) %>% # Distinct only shows the factorized values of those values
  dplyr::arrange(gs_cat, gs_subcat) # Arrange values in order
...
gseaplot2(myGSEA.res, 
          geneSetID = c(30,104,300), #can choose multiple signatures to overlay in this plot
          pvalue_table = FALSE, #can set this to FALSE for a cleaner plot
          title = myGSEA.res$Description[104]) #can also turn off this title

# add a variable to this result that matches enrichment direction with phenotype
myGSEA.df <- myGSEA.df %>%
  mutate(phenotype = case_when(
    NES > 0 ~ "disease", # Pathway enriched in patients with disease
    NES < 0 ~ "healthy")) # Enriched in healthy controls
...
# both results in mroast are significant
...
broadSet.C2.ALL <- getGmt("GSEA/c2.all.v7.1.symbols.gmt", geneIdType=SymbolIdentifier())
#extract as a list
