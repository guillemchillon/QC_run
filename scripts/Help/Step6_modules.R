...
# IMPORTANT INFORMATION BETWEEN SPEARMAN AND PEARSON CORRELATIONS----
## Difference between pearson and Spearman:
### Depending on the relationship we use, the groups will change:
### Pearson will cluster the data
### Spearman will take a rank ordering of that data and operate of the ranks to 
#### to figure out the correlation
## Spearman will be less sensitive to magnitude, less influence by dramatic changes
### We do not want a highly expressed genes to skew the difference of our samples
### That's why we use Spearman correlation for samples but Pearson correlation for our genes.
...
#what happens when you change scale='none'-> It is not normalized

# IMPORTANT INFORMATION ABOUT SCALING----
## Scale = "row" --> emphasis on comparing between samples
## Scale = "none" --> emphasis comparing between genes
### Since our interest is comparing samples, we use scale = "row"
...
modulePick <- 2 #use 'c()' to grab more than one cluster from the heatmap.  e.g., c(1,2)
                # Module 1=Genes that went down in infection
                # Module 2=Genes that went up in infection  
