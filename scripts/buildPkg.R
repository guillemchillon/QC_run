...
available("PackageDIYtranscriptomics", browse = FALSE)

# running the 'create_package()' function will create an R package and open a new Rsession in that package
create_package("~/Documents/PhD/1st Year/UPenn/Spring/CAMB 7140/PackageDIYtranscriptomics") 
...
setwd("~/Documents/PhD/1st Year/UPenn/Spring/CAMB 7140/PackageDIYtranscriptomics")
...
use_mit_license("Guillem Chillon")
...
build() # probably best to run in the RStudio session for your new package (Build source package)

# Autenticate to use GitHub ----
# You only need to do this authentication once
use_git_config(user.name = "guillemchillon", user.email = "chillonguillem@gmail.com")
git_sitrep()
...
use_github_action() #adds a github actions workflow to your package
use_coverage() #adds a code coverage badge to your README
