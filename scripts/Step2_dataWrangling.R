...
colSums(myTPM) # These number are close to a million, but are not a million. This is because we are looking at gene level data, not at transcript level data (if looking at the transcript, we should get 1000000 reads).
colSums(myCounts)
...
cpm <- cpm(myDGEList) 
colSums(cpm)
log2.cpm <- cpm(myDGEList, log=TRUE) # TRUE will log2 the DGEs
...
log2.cpm.df.pivot
DIYprofile(data = myDGEList, samples = sampleLabels)
DIYprofile(data = myDGEList.filtered, samples = sampleLabels)
DIYprofile(data = myDGEList.filtered.norm, samples = sampleLabels)
